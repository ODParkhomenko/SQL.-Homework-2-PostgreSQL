...
create table if not exists Albums (
	id_album integer primary key references Artists(id_artist),
	album_name varchar (100) not null,
	album_release_year date not null
);

create table if not exists Tracks (
	id_track integer primary key references Albums(id_album),
...
create table if not exists Collection_Tracks (
	id_collection integer references Сollections(id_collection) not null,
